...
export const pageTools: ToolDefinition[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Optional status filter. Omit to return all statuses.' },
        tag: { type: 'string', description: 'Optional tag filter. Omit to skip tag filtering.' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        slug: { type: 'string', description: 'URL path slug to match (e.g. "getting-started/intro"). Mutually exclusive with pageId.' },
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Optional status filter.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        projectRoot: PROJECT_ROOT_SCHEMA_FIELD,
        sourceLang: { type: 'string', description: 'Source language code. Must be an enabled project language.' },
        targetLang: { type: 'string', description: 'Target language code to clone into. Must be an enabled project language.' },
        sourcePageId: PAGE_ID_SCHEMA_FIELD,
        includeContent: { type: 'boolean', description: 'If true, copies the source content blocks into the clone. Defaults to false (skeleton only).' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        projectRoot: PROJECT_ROOT_SCHEMA_FIELD,
        sourceLang: { type: 'string', description: 'Reference language code for the pairing comparison.' },
        targetLang: { type: 'string', description: 'Target language code for the pairing comparison.' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        template: {
          type: 'object',
          description: 'Template definition to create or update.',
          properties: {
            id: { type: 'string' },
            label: {
              oneOf: [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        projectRoot: PROJECT_ROOT_SCHEMA_FIELD,
        templateId: { type: 'string', description: 'Optional template id to inspect. Omit to list all available templates.' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        slug: { type: 'string', description: 'URL path for this page (e.g. "getting-started/intro"). Must be unique within the language.' },
        title: { type: 'string', description: 'Display title of the page.' },
        description: { type: 'string', description: 'Short description shown in listings and meta tags. Optional.' },
        template: { type: 'string', description: 'Template id to use for content generation. Use page_template_query to list available templates. Optional.' },
        metadata: { type: 'object', description: 'Key-value metadata matching the template\'s metadataSchema, if any. Optional.' },
        tags: { type: 'array', items: { type: 'string' }, description: 'Tag labels for filtering and categorization. Optional.' },
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Publication status. Only "published" pages appear in reader/search/llms.txt artifacts. Defaults to "draft".' },
        content: { type: 'object', description: 'Page content in doc-content-v1 format: { "version": 1, "blocks": [...] }. Omit to create a blank page or let the template generate content.' },
        render: { type: 'object', description: 'Pre-computed render artifacts (markdown, plainText). Normally omitted — the service generates these automatically.' },
        review: { type: 'object', description: 'Review metadata. Optional.' },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        slug: { type: 'string', description: 'URL path for this page. Must be unique within the language.' },
        markdown: { type: 'string', description: 'Markdown or MDX source text to convert and store as doc-content-v1.' },
        inputMode: { type: 'string', enum: ['document', 'fragment'], description: '"document" treats the input as a full page (parses frontmatter, uses H1 as title). "fragment" treats it as a content snippet. Defaults to "document".' },
        format: { type: 'string', enum: ['markdown', 'mdx'], description: 'Source format. Defaults to "markdown".' },
        sourcePath: { type: 'string', description: 'Original file path hint for error attribution. Optional.' },
        title: { type: 'string', description: 'Override for the page title (otherwise inferred from H1 or frontmatter).' },
        description: { type: 'string', description: 'Override for the page description (otherwise inferred from frontmatter). Optional.' },
        template: { type: 'string', description: 'Template id for metadata scaffolding. Optional.' },
        metadata: { type: 'object' },
        tags: { type: 'array', items: { type: 'string' }, description: 'Tag labels. Optional.' },
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Publication status. Defaults to "draft".' },
        review: { type: 'object' },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        slug: { type: 'string', description: 'URL path for this page (e.g. "getting-started/intro"). Must be unique within the language.' },
        title: { type: 'string', description: 'Display title of the page.' },
        template: { type: 'string', description: 'Template id used to generate the page content. Use page_template_query to list available templates.' },
        description: { type: 'string', description: 'Short description shown in listings and meta tags. Optional.' },
        metadata: { type: 'object', description: 'Key-value metadata matching the template\'s metadataSchema, if any. Optional.' },
        tags: { type: 'array', items: { type: 'string' }, description: 'Tag labels for filtering and categorization. Optional.' },
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Publication status. Only "published" pages appear in reader/search/llms.txt artifacts. Defaults to "draft".' },
        review: { type: 'object', description: 'Review metadata. Optional.' },
        summary: { type: 'string', description: 'Overview paragraph used by concept/how_to/reference templates. Falls back to template defaultSummary if omitted.' },
        sections: { type: 'array', items: { type: 'object' }, description: 'Structured content sections for the template (each has title, optional body/items/code/language/callout). Falls back to template defaultSections if omitted.' },
        steps: { type: 'array', items: { type: 'object' }, description: 'Ordered step objects for how_to templates (each has title, optional body/code/language).' },
        callouts: { type: 'array', items: { type: 'object' }, description: 'Callout objects to append (each has body, optional title and theme: info|warning|success).' },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        template: { type: 'string', description: 'Template id used to regenerate the page content. Use page_template_query to list available templates.' },
        slug: { type: 'string', description: 'URL path override. Must be unique within the language. Optional.' },
        title: { type: 'string', description: 'Display title override. Optional.' },
        description: { type: 'string', description: 'Short description override. Optional.' },
        metadata: { type: 'object', description: 'Key-value metadata matching the template\'s metadataSchema, if any. Optional.' },
        tags: { type: 'array', items: { type: 'string' }, description: 'Tag labels for filtering and categorization. Optional.' },
        review: { type: 'object', description: 'Review metadata. Optional.' },
        summary: { type: 'string', description: 'Overview paragraph used by concept/how_to/reference templates. Falls back to template defaultSummary if omitted.' },
        sections: { type: 'array', items: { type: 'object' }, description: 'Structured content sections for the template. Falls back to template defaultSections if omitted.' },
        steps: { type: 'array', items: { type: 'object' }, description: 'Ordered step objects for how_to templates. Optional.' },
        callouts: { type: 'array', items: { type: 'object' }, description: 'Callout objects to append (each has body, optional title and theme: info|warning|success). Optional.' },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        pages: {
          type: 'array',
          items: { type: 'object' },
          description: 'Array of page create payloads. Each item has the same shape as page_create arguments (pageId, slug, title, etc.).',
        },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        patch: {
          type: 'object',
          description:
            'Partial update object. Supported fields: slug, title, description, template, metadata, tags, content, render, review. Only provided fields are updated.',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        markdown: { type: 'string', description: 'Markdown or MDX source text to convert and apply to the page.' },
        operation: { type: 'string', enum: ['replace', 'append'], description: '"replace" overwrites the existing content. "append" adds the converted blocks after the existing content. Defaults to "replace".' },
        inputMode: { type: 'string', enum: ['document', 'fragment'], description: '"document" parses the input as a full page (frontmatter + H1). "fragment" treats it as a snippet. Defaults to "document".' },
        format: { type: 'string', enum: ['markdown', 'mdx'], description: 'Source format. Defaults to "markdown".' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: LANG_SCHEMA_FIELD,
        pageId: PAGE_ID_SCHEMA_FIELD,
        status: { type: 'string', enum: ['draft', 'in_review', 'published'], description: 'Target status to set on the page.' },
        dryRun: DRY_RUN_SCHEMA_FIELD,
      },
