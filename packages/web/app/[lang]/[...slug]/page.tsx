...
export default async function Page({
  ...
  // OpenAPI-backed pages: render the self-built reference overview inline.
  // Operation detail lives on the dedicated /reference route (one URL per operation).
  if (page.template === "api-source") {
    ...
    const doc = await getPublishedOpenApiDoc(
      lang,
      getApiSourceRouteSlug(apiSource),
      source.projectId,
      source.customPath,
    );
    if (!doc) notFound();
