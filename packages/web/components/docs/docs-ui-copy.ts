...
const DOCS_UI_COPY: Record<DocsLang, DocsUiCopy> = {
  en: {
    ...
    landing: {
      ...
      quickStartDescription: 'These pages give you the fastest path into the documentation.',
      ...
      keepExploringDescription: 'After the first pages, these are the next documents worth opening.',
  ...
  zh: {
    ...
    landing: {
      eyebrow: '经典文档',
      title: '先从定义结构的页面开始阅读',
      description: '左侧目录保留完整的信息结构，这里只前置最值得先读的页面和主要分类入口。',
      searchLabel: '快速搜索',
      quickStartTitle: '开始阅读',
      quickStartDescription: '先读这些页面，能最快建立对整套文档的认识。',
      categoriesTitle: '按分类浏览',
      categoriesDescription: '首页只提取少量常用入口，完整目录仍然保持在左侧导航里。',
