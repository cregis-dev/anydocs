...
export const studioTest = base.extend<{
  ...
  studioPage: async ({ page }, applyFixture) => {
    const studioPage = new StudioPage(page);
    await applyFixture(studioPage);
  },
});
...
export class StudioPage {
  ...
  async waitForStudio() {
    await this.page
      .waitForSelector("text=PAGES", { timeout: 10000 })
      .catch(() => {
  ...
  getSaveStatus(): Locator {
    return this.page
      .locator("footer")
      .locator("text=All changes saved, text=Unsaved changes, text=Saving...");
  }
